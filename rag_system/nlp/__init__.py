...
def bullets_category(sections):
    ...
    for i, pro in enumerate(BULLET_PATTERN):
        for sec in sections:
            # 确保 sec 是字符串
            sec_text = str(sec).strip()
            for p in pro:
                if re.match(p, sec_text) and not not_bullet(sec_text):
                    hits[i] += 1
                    # 找到一个匹配就可以跳到下一个模式了，提高效率
                    break

    maxium = 0
    res = -1
    for i, h in enumerate(hits):
        if h > maxium:  # 注意：这里用 > 而不是 >= 来确保稳定性
            res = i
            maxium = h

    if res == -1:
        # 如果没有找到任何匹配的标题模式，返回一个默认的元组
        logging.debug("bullets_category 未找到匹配的标题模式。")
        return -1, ""
    else:
        # 如果找到了最佳模式，返回它的索引和模式列表（元组形式）
        logging.debug(f"bullets_category 找到最佳标题模式: 索引 {res}")
        return res, BULLET_PATTERN[res]
